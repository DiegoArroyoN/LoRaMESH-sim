#!/usr/bin/env python3
"""
plot_loss_grouped_box.py  (v4 — exact per-data-packet counters)
Uses data_collision_drops / data_busy_drops from the new simulator fields
when available; falls back to the proportional-split estimation for old
JSON files that pre-date the patch.

Grouped boxplots per N: one box per *active* loss cause (causes with max
value > 0.5% across all seeds/N are shown; others are skipped so boxes
are wide enough to see clearly).
Legend placed below x-axis, 3 columns.
"""
...
ALL_CATS   = ['collision', 'rx_busy', 'no_route', 'ttl', 'queue_full', 'other']
ALL_COLORS = ['#C0392B', '#E67E22', '#2471A3', '#8E44AD', '#1E8449', '#95A5A6']
...
def extract(path):
    ...
    # v4 exact counters (present when binary was built after the patch)
    ...
    if exact_col is not None and exact_busy is not None:
        ...
        # Legacy estimation: split PHY residual by raw event-count ratio
...
def active_categories(data):
    """Return only categories whose max value across all N/seeds >= threshold."""
    cats, colors, labels = [], [], []
