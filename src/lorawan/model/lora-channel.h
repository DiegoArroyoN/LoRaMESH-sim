...
{
    double rxPowerDbm{-200.0}; //!< The reception power.
    uint8_t sf{7};             //!< The Spreading Factor of this transmission.
    Time duration{Seconds(0)}; //!< The duration of the transmission.
    uint32_t frequencyHz{0};   //!< The frequency [Hz] of this transmission.
    uint64_t signalId{0};      //!< Opaque signal identifier for latent PHY context.
};
...
class LoraChannel : public Channel
{
  public:
    ...
    std::vector<TxEvent> GetTxEvents(Time now) const;
    ...
    /**
     * Retrieve latent SF context for a signal id.
     *
     * This API is meant for PHY internal detection logic where SF is treated as
     * an environment latent variable until lock.
     *
     * @param signalId Opaque signal identifier.
     * @param sfOut Output spreading factor when found.
     * @return true if context exists, false otherwise.
     */
    bool GetLatentSignalSf(uint64_t signalId, uint8_t* sfOut) const;
  ...
  private:
    ...
    struct LatentSignalInfo
    {
        uint8_t sf{7};
        Time expiresAt{Seconds(0)};
    };

    uint64_t RegisterLatentSignal(uint8_t sf, Time expiresAt) const;
    void PurgeLatentSignals(Time now) const;
    ...
    mutable std::vector<TxEvent> m_txEvents;
    mutable uint64_t m_nextSignalId{1};
    mutable std::unordered_map<uint64_t, LatentSignalInfo> m_latentSignals;
};
