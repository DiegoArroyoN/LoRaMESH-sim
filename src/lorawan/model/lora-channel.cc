                  ...
                  uint32_t frequencyHz) const
{
    ...
    PurgeLatentSignals(now);
    if (duration.IsPositive())
    {
        auto endTime = now + duration;
        m_txEvents.push_back(
            {now, endTime, frequencyHz, txParams.sf, sender, txPowerDbm, txParams.bandwidthHz});
    }
    m_txEvents.erase(std::remove_if(m_txEvents.begin(),
                                    m_txEvents.end(),
                                    [now](const TxEvent& ev) { return ev.endTime <= now; }),
                     m_txEvents.end());
    ...
    for (i = m_phyList.begin(); i != m_phyList.end(); i++, j++)
    {
        ...
        if (sender != (*i))
        {
            ...
            parameters.signalId = RegisterLatentSignal(txParams.sf, now + delay + duration);
...
std::vector<LoraChannel::TxEvent>
LoraChannel::GetTxEvents(Time now) const
{
    if (!m_txEvents.empty())
    {
        m_txEvents.erase(std::remove_if(m_txEvents.begin(),
...
LoraChannel::Receive(uint32_t i, Ptr<Packet> packet, LoraChannelParameters parameters) const
{
    ...
    Ptr<SimpleGatewayLoraPhy> simpleGw = DynamicCast<SimpleGatewayLoraPhy>(m_phyList[i]);
    if (simpleGw)
    {
        // Scan-based PHY path: no public SF exposure before lock.
        simpleGw->NotifySignalStart(packet,
    ...
    // Legacy PHY path.
    m_phyList[i]->StartReceive(
        packet, parameters.rxPowerDbm, parameters.sf, parameters.duration, parameters.frequencyHz);
}
...
bool
LoraChannel::GetLatentSignalSf(uint64_t signalId, uint8_t* sfOut) const
{
    PurgeLatentSignals(Simulator::Now());
    auto it = m_latentSignals.find(signalId);
    if (it == m_latentSignals.end())
    {
        return false;
    }
    if (sfOut)
...
uint64_t
LoraChannel::RegisterLatentSignal(uint8_t sf, Time expiresAt) const
{
    const uint64_t signalId = m_nextSignalId++;
    LatentSignalInfo info;
    info.sf = sf;
    info.expiresAt = expiresAt;
    m_latentSignals.emplace(signalId, info);
    return signalId;
}

void
LoraChannel::PurgeLatentSignals(Time now) const
{
    if (m_latentSignals.empty())
    {
        return;
    }
    for (auto it = m_latentSignals.begin(); it != m_latentSignals.end();)
    {
        if (it->second.expiresAt <= now)
        {
            it = m_latentSignals.erase(it);
        }
        else
        {
            ++it;
