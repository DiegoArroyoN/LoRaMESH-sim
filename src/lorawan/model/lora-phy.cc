...
LoraPhy::GetOnAirTime(Ptr<Packet> packet, LoraTxParameters txParams)
{
    ...
    double plD = static_cast<double>(pl);
    double sfD = static_cast<double>(txParams.sf);
    double crD = static_cast<double>(txParams.codingRate);

    // num and den refer to numerator and denominator of the time on air formula
    double num = 8.0 * plD - 4.0 * sfD + 28.0 + 16.0 * crc - 20.0 * h;
    double den = 4.0 * (sfD - 2.0 * de);
    double payloadSymbNb = 8.0 + std::max(std::ceil(num / den) * (crD + 4.0), 0.0);
