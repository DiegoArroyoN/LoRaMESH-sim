                                     ...
                                     uint32_t frequencyHz)
{
    ...
    case STANDBY: {
        ...
        // ★ NUEVO: Log ANTES de las checks
        NS_LOG_INFO("*** STANDBY case: rxPower=" << rxPowerDbm 
            << " dBm, sensitivity=" << sensitivity
            << ", freq=" << frequencyHz << " Hz, myFreq=" << m_frequencyHz
            << ", SF=" << unsigned(sf) << ", mySF=" << unsigned(m_sf));
        // Check frequency
        //////////////////
...
SimpleEndDeviceLoraPhy::EndReceive(Ptr<Packet> packet, Ptr<LoraInterferenceHelper::Event> event)
{
    ...
    NS_LOG_INFO("*** EndReceive() CALLED - this=" << this << " node=" << (m_device ? m_device->GetNode()->GetId() : -1));

    // Automatically switch to Standby in either case
    SwitchToStandby();
    NS_LOG_INFO("*** EndReceive() CALLED for packet - node=" << (m_device ? m_device->GetNode()->GetId() : -1));
    ...
    else
    {
        ...
        {   
            NS_LOG_INFO("*** m_rxOkCallback is NOT NULL - invoking callback");  // ← NUEVO
            LoraTag tag;
            packet->RemovePacketTag(tag);
        ...
        {
            NS_LOG_WARN("*** m_rxOkCallback is NULL");  // ← NUEVO
        }
    }
}
} // namespace lorawan
