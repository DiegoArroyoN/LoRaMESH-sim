...
LoraMeshHelper::InstallDevices(NodeContainer& nodes)
{
    ...
    NS_LOG_UNCOND("✓ Canal LoRa creado con LogDistancePropagationLossModel (ref="
    ...
    if (m_enablePcap)
    {
        ...
        lorawan::MeshLoraNetDevice::InitGlobalPcap(pcapName);
    }
...
LoraMeshHelper::InstallApplications(NodeContainer& nodes)
{
    ...
    for (uint32_t i = 0; i < nodes.GetN(); ++i)
    {
        ...
        NS_LOG_INFO(">>> MeshDvApp installed on node "
...
LoraMeshHelper::ForceStandbyMode(NodeContainer& nodes)
{
    ...
    for (uint32_t i = 0; i < nodes.GetN(); ++i)
    {
        ...
        Ptr<lorawan::MeshLoraNetDevice> meshDev = DynamicCast<lorawan::MeshLoraNetDevice>(netDev);
        ...
        if (meshDev)
        {
            ...
            if (phy)
            {
                ...
                NS_LOG_UNCOND("✓ Node " << i << " PHY (single-demod) ready for RX");
            }
        }
    }

    NS_LOG_UNCOND("=== TODOS LOS NODOS LISTOS PARA RX ===");
}
