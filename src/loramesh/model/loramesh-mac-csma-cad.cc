    ...
    static TypeId tid =
        TypeId("ns3::loramesh::CsmaCadMac")
            ...
            .AddAttribute("CadBandwidthHz",
                          ...
                          MakeUintegerAccessor(&CsmaCadMac::m_cadBandwidthHz),
                          MakeUintegerChecker<uint32_t>(1))
            .AddAttribute("CadDurationMode",
                          "CAD duration mode: fixed | sf_bw.",
                          EnumValue(CsmaCadMac::CAD_DURATION_SF_BW),
                          MakeEnumAccessor<CsmaCadMac::CadDurationMode>(
                              &CsmaCadMac::m_cadDurationMode),
                          MakeEnumChecker(CsmaCadMac::CAD_DURATION_FIXED,
                                          "fixed",
                                          CsmaCadMac::CAD_DURATION_SF_BW,
                                          "sf_bw"))
...
CsmaCadMac::NotifyTxStart(double toaSeconds)
{
    ...
    NS_LOG_DEBUG("CsmaCadMac: TX recorded duration=" << duration.GetSeconds() << "s dc="
...
Time
CsmaCadMac::GetDutyCycleWindow() const
{
...
uint32_t
CsmaCadMac::GetMaxBackoffSlots() const
{
    return m_maxBackoffSlots;
}

void
CsmaCadMac::SetMaxBackoffSlotsAbs(uint32_t slots)
{
    m_maxBackoffSlotsAbs = std::max<uint32_t>(1, slots);
}

uint32_t
CsmaCadMac::GetMaxBackoffSlotsAbs() const
{
...
void
CsmaCadMac::SetPhy(Ptr<lorawan::LoraPhy> phy)
{
    m_phy = phy;
}

void
CsmaCadMac::SetCadContext(uint8_t sf, uint32_t bwHz)
{
    if (sf >= 7 && sf <= 12)
    {
        m_cadSf = sf;
...
void
CsmaCadMac::UpdateTypicalDataToaSeconds(double toaSeconds)
{
    UpdateToaEma(m_toaEmaDataSeconds, toaSeconds);
}

void
CsmaCadMac::UpdateTypicalCtrlToaSeconds(double toaSeconds)
{
    UpdateToaEma(m_toaEmaCtrlSeconds, toaSeconds);
}

void
CsmaCadMac::UpdateToaEma(double& emaSeconds, double toaSeconds)
{
    if (toaSeconds <= 0.0)
    {
        return;
...
uint32_t
CsmaCadMac::GetFailureCount() const
{
    return m_failures;
}

uint32_t
CsmaCadMac::GetLastBackoffSlots() const
{
    return m_lastBackoffSlots;
}

uint32_t
CsmaCadMac::GetLastBackoffWindowSlots() const
{
    return m_lastBackoffWindowSlots;
}

double
CsmaCadMac::GetCadLoadEstimate() const
{
    return GetCadLoad();
}

uint64_t
CsmaCadMac::GetCadBusyEventsLocalPower() const
{
    return m_cadBusyEventsLocalPower;
}

uint64_t
CsmaCadMac::GetCadBusyEventsOracle() const
{
...
CsmaCadMac::PerformChannelAssessment()
{
    ...
    for (uint8_t i = 0; i < m_difsCadCount; ++i)
    {
        ...
        {
            NS_LOG_INFO("CsmaCadMac: CAD busy node="
...
bool
CsmaCadMac::PerformCadOnce(Time sampleTime)
{
    Ptr<lorawan::LoraChannel> channel = m_phy ? m_phy->GetChannel() : nullptr;
    if (!channel)
    {
        NS_LOG_WARN("CsmaCadMac: no PHY/channel attached, CAD defaults to busy");
    ...
    for (const auto& ev : events)
    {
        ...
        {
            NS_LOG_DEBUG("CsmaCadMac: CAD(oracle) busy by active tx on freq="
        ...
        if (!m_phy || !ev.sender)
        {
            NS_LOG_WARN("CsmaCadMac: missing PHY/sender for local_power CAD, fallback busy");
        ...
        {
            NS_LOG_WARN("CsmaCadMac: missing mobility for local_power CAD, fallback busy");
            m_cadBusyEventsLocalPower++;
            RecordCadResult(true);
            return true;
        }
        ...
        {
            NS_LOG_DEBUG("CsmaCadMac: CAD(local_power) busy freq="
                         << ev.frequencyHz << " sf=" << unsigned(ev.sf)
    ...
    NS_LOG_DEBUG("CsmaCadMac: CAD result busy=false");
...
void
CsmaCadMac::RecordCadResult(bool busy)
{
...
uint32_t
CsmaCadMac::ComputeBackoffWindowSlots() const
{
...
uint32_t
CsmaCadMac::ComputeMaxBackoffSlots() const
{
...
Time
CsmaCadMac::ComputeCadDurationFromContext() const
{
...
double
CsmaCadMac::GetCadSensitivityDbm(uint8_t sf) const
{
