...
EnergyModel::RegisterNode (NodeId id)
{
  ...
  NS_LOG_INFO ("EnergyModel: Node " << id << " re-registered keeping remaining="
               << it->second.remainingMah << "mAh");
}

void
EnergyModel::SetRemainingFraction (NodeId id, double fraction)
{
...
void
EnergyModel::UpdateCadEnergy (NodeId id, double durationSeconds)
{
  NodeEnergyState& state = EnsureNode (id);
  ApplyIdleConsumption (state);
  ...
  state.remainingMah = std::max (0.0, state.remainingMah - consumedMah);
  state.lastUpdate = Simulator::Now ();
  NS_LOG_DEBUG ("EnergyModel: Node " << id << " CAD consumed=" << consumedMah
                << "mAh remaining=" << state.remainingMah);
}
...
void
EnergyModel::SetCadCurrentMa (double current)
{
