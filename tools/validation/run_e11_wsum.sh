#!/bin/bash
# run_e11_wsum.sh — los pesos a SUMA CONSTANTE, y la escala por separado.
#
...
OUT=$HOME/ns3-runs/e11_$CHAN
mkdir -p "$OUT/cells"
CSV="$OUT/e11_wsum.csv"
...
cell() {
  ...
  ("$BIN" --profile=proposal_pueyo_like_csmacad --nEd="$n" --stopSec=$stop --rngRun="$seed" \
      ...
      --shadowingModel="$CHAN" --enablePcap=false --verboseLogs=false $extra $mf > run.log 2>&1) 2>/dev/null
  local rc=$?
...
if [ -f "$HOME/assert_config.sh" ]; then
    bash "$HOME/assert_config.sh" "$BIN" /tmp/cfgchk_$$ \
        ...
        sfmin=7 sfmax=12 wire=pueyo7b hyst=1 shadow=$CHAN spacing=178 coststep=0.025 \
...
B d00    0.85 0.15 0.00
B d05    0.80 0.15 0.05
B d15    0.70 0.15 0.15
B d25    0.60 0.15 0.25
C k025   0.15 0.0375 0.0625
C k050   0.30 0.075 0.125
C k200   1.20 0.30 0.50
C k400   2.40 0.60 1.00
R toaref 0 0 0
"
...
echo "== E11_DONE: $(( $(wc -l < "$CSV") - 1 )) filas =="
