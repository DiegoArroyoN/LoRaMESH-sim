...
std::string
FormatDecodedEntries(const std::vector<ns3::loramesh::DvEntry>& entries)
{
    std::ostringstream oss;
    for (size_t i = 0; i < entries.size(); ++i)
    {
...
MeshDvApp::MeshDvApp()
{
    ...
    m_routing->SetLocalEnergyFractionCallback(MakeCallback(&MeshDvApp::GetEnergyFraction, this));
    ...
    static TypeId tid =
        TypeId("ns3::MeshDvApp")
            ...
            .AddAttribute("TrafficMode",
                          ...
                          MakeStringAccessor(&MeshDvApp::SetTrafficMode, &MeshDvApp::GetTrafficMode),
                          MakeStringChecker())
            .AddAttribute("SfLinkMode",
                          ...
                          MakeStringAccessor(&MeshDvApp::SetSfLinkMode, &MeshDvApp::GetSfLinkMode),
                          MakeStringChecker())
            .AddAttribute("SfLinkMarginDb",
                          "Margin [dB] added over sensitivity when SfLinkMode=deterministic_sensitivity.",
                          DoubleValue(0.0),
                          MakeDoubleAccessor(&MeshDvApp::m_sfLinkMarginDb),
            ...
            .AddAttribute("PueyoPacketsPerPair",
                          ...
                          UintegerValue(100),
                          MakeUintegerAccessor(&MeshDvApp::m_pueyoPacketsPerPair),
            ...
            .AddAttribute("BeaconLatestOnly",
                          ...
                          BooleanValue(true),
                          MakeBooleanAccessor(&MeshDvApp::m_beaconLatestOnly),
                          MakeBooleanChecker())
            .AddAttribute("PueyoStrictQueueScheduler",
                          "Enable strict queue scheduler (Routing/Data=10:1, Forward/Local=10:1).",
                          BooleanValue(false),
                          MakeBooleanAccessor(&MeshDvApp::m_usePueyoStrictQueueScheduler),
                          MakeBooleanChecker())
            ...
            .AddAttribute("DvPayloadMaxBytes",
                          ...
                          UintegerValue(0),
                          MakeUintegerAccessor(&MeshDvApp::m_dvPayloadMaxBytes),
                          MakeUintegerChecker<uint32_t>())
            .AddAttribute("RouteAdvertPolicy",
                          ...
                          MakeStringAccessor(&MeshDvApp::m_routeAdvertPolicy),
                          MakeStringChecker())
...
void
MeshDvApp::SetTrafficMode(std::string mode)
{
...
void
MeshDvApp::SetSfLinkMode(std::string mode)
{
    const std::string normalized = ToLower(mode);
...
std::string
MeshDvApp::GetTrafficMode() const
{
...
void
MeshDvApp::BuildPueyoTrafficSchedule()
{
...
void
MeshDvApp::CountDropNoRouteRelay()
{
    m_dropNoRoute++;
    m_dropNoRouteRelay++;
}
...
void
MeshDvApp::BuildAndSendDvPueyo(uint8_t sf)
{
    if (!m_enableDvBroadcast)
    {
        return;
    ...
    if (m_routing)
    {
        ...
        {
            NS_LOG_UNCOND("PUEYO_VAL_TX_INTERNAL node=" << GetNode()->GetId()
    ...
    if (!entries.empty())
    {
        ...
        if (m_pueyoValidationTrace)
        {
            NS_LOG_UNCOND("PUEYO_VAL_TX_PAYLOAD node=" << GetNode()->GetId()
                                                       << " payload_hex="
    ...
    NS_LOG_UNCOND("DVTRACE_TX_PUEYO time=" << Simulator::Now().GetSeconds()
                                           << " node=" << GetNode()->GetId()
...
MeshDvApp::StartApplication()
{
    ...
    if (m_enableGapAuditTrace)
    {
        ...
        Simulator::Schedule(Seconds(dataStartSec),
                            &MeshDvApp::CaptureControlPlaneAuditSnapshot,
                            this);
        if (m_dataStopTimeSec > dataStartSec)
        {
            ...
            Simulator::Schedule(Seconds(midpointSec),
                                &MeshDvApp::CaptureControlPlaneAuditMidpointSnapshot,
                                this);
            Simulator::Schedule(Seconds(m_dataStopTimeSec),
                                &MeshDvApp::CaptureControlPlaneAuditDataStopSnapshot,
                                this);
        }
    }
}
...
MeshDvApp::StopApplication()
{
    ...
    if (g_metricsCollector)
    {
        ...
        if (m_routing)
        {
            ...
            {
                g_metricsCollector->RecordQuantizationSample(GetNode()->GetId(),
                                                             r.destination,
                                                             r.nextHop,
                                                             false,
                                                             r.rawMetric,
...
void
MeshDvApp::AccountRxScanEnergyDelta()
{
...
uint8_t
MeshDvApp::ResolveSfForLink(uint8_t observedSf, double rxPowerDbm) const
{
...
void
MeshDvApp::UpdateControlPlaneAuditState()
{
    if (!m_enableGapAuditTrace)
    {
        return;
    ...
    m_gapAuditEvt = Simulator::Schedule(Seconds(1), &MeshDvApp::UpdateControlPlaneAuditState, this);
}
...
bool
MeshDvApp::IsControlQueueEntry(const TxQueueEntry& entry) const
{
    return entry.tag.GetDst() == 0xFFFF;
}

bool
MeshDvApp::IsForwardedDataQueueEntry(const TxQueueEntry& entry) const
{
    Ptr<Node> node = GetNode();
...
void
MeshDvApp::ResetStrictRoutingDataBudgets()
{
...
void
MeshDvApp::ResetStrictForwardLocalBudgets()
{
...
void
MeshDvApp::RotateQueueEntryToFront(std::size_t idx)
{
...
bool
MeshDvApp::SelectStrictQueueHead(std::size_t* outIndex)
{
        ...
        NS_LOG_INFO("CSMA: TX ok=" << ok);
            if (ok)
            {
                ...
                if (m_wireFormat != "v1" && entry.beaconRpCounterAssigned)
                {
                    ...
                    NS_LOG_UNCOND(traceLabel << " time=" << Simulator::Now().GetSeconds()
                                             << " node=" << GetNode()->GetId()
                    ...
                    if (m_pueyoValidationTrace && m_wireFormat == "pueyo7b")
                    {
                        ...
                        NS_LOG_UNCOND("PUEYO_VAL_TX_AIR node=" << GetNode()->GetId()
                                                               << " packet_hex="
                            ...
                            uint32_t toaUsNeighbor)
{
    ...
    loramesh::NeighborLinkInfo link = BuildNeighborLinkInfo(tag, toaUsNeighbor, fromMac, linkSf);
...
bool
MeshDvApp::ParseDataWirePacketPueyo7b(Ptr<const Packet> p,
                                      DataWireHeaderPueyo7b* outHdr,
                                      Ptr<Packet>* outPayload) const
{
    if (!outHdr || !p || p->GetSize() < DataWireHeaderPueyo7b::kSerializedSize)
    {
        return false;
    }

    Ptr<Packet> copy = p->Copy();
    DataWireHeaderPueyo7b hdr;
    const uint32_t removed = copy->RemoveHeader(hdr);
    if (removed != DataWireHeaderPueyo7b::kSerializedSize)
    {
        return false;
    }
    if (GetPacketTypeV2(hdr.GetFlagsTtl()) != WirePacketTypeV2::DATA)
...
bool
MeshDvApp::ParseBeaconWirePacketPueyo(Ptr<const Packet> p,
                                      BeaconWireHeaderV2* outHdr,
                                      Ptr<Packet>* outPayload) const
{
...
std::vector<loramesh::DvEntry>
MeshDvApp::DecodeDvEntriesPueyo(Ptr<const Packet> p,
                                uint32_t payloadOffset,
                                uint32_t toaUsNeighbor,
                                uint8_t rxSf) const
{
    ...
    if (m_pueyoValidationTrace)
    {
        NS_LOG_UNCOND("PUEYO_VAL_RX_DECODE node=" << GetNode()->GetId()
                                                  << " payload_hex="
...
MeshDvApp::L2ReceiveV2(Ptr<NetDevice> dev, Ptr<const Packet> p, uint16_t proto, const Address& from)
{
    ...
    if (isPueyoBeacon)
    {
        ...
        if (m_pueyoValidationTrace && m_wireFormat == "pueyo7b")
        {
            NS_LOG_UNCOND("PUEYO_VAL_RX_MSG node=" << myId << " origin=" << src
...
void
MeshDvApp::LogTxEvent(uint32_t src,
                      uint32_t seq,
                      uint32_t dst,
                      uint8_t ttl,
                      uint8_t hops,
...
void
MeshDvApp::SendDataPacketPueyo7b(uint32_t dst)
{
    TrackActiveDestination(dst);
    const uint32_t myId = GetNode()->GetId();
    const uint16_t seq16 = static_cast<uint16_t>((++m_dataSeqPerNode) & 0xFFFF);
