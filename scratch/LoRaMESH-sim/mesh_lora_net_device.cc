...
#include "beacon_wire_header_pueyo.h"
...
MeshLoraNetDevice::GetTypeId()
{
    static TypeId tid = TypeId("ns3::lorawan::MeshLoraNetDevice")
                            ...
                            .AddAttribute("PreambleSymbols",
                                          ...
                                          UintegerValue(8),
                                          MakeUintegerAccessor(&MeshLoraNetDevice::m_preambleSymbols),
...
MeshLoraNetDevice::Send(Ptr<Packet> packet, const Address& dest, uint16_t protocolNumber)
{
    ...
    {
        double txCurrentMa = LoRaDeviceEnergyModel::EstimateTxCurrentA(txPowerDbm) * 1000.0;
        if (m_loraEnergyModel)
        {
...
MeshLoraNetDevice::Receive(Ptr<const Packet> packet)
{
    ...
    else if (m_wireFormat != "v1" && pktForUpper->GetSize() >= BeaconWireHeaderV2::kSerializedSize)
    {
        ...
        uint16_t logicalSrc = 0;
        uint8_t flagsTtl = 0;
        bool parsedBeacon = false;
        if (m_wireFormat == "pueyo7b")
        {
            BeaconWireHeaderPueyo beaconHdr;
            const uint32_t removed = hdrProbe->RemoveHeader(beaconHdr);
            if (removed == BeaconWireHeaderPueyo::kSerializedSize &&
                GetPacketTypeV2(beaconHdr.GetFlagsTtl()) == WirePacketTypeV2::BEACON)
            {
                logicalSrc = beaconHdr.GetSrc();
                flagsTtl = beaconHdr.GetFlagsTtl();
                parsedBeacon = true;
            }
        }
        else
        ...
        if (parsedBeacon && GetPacketTypeV2(flagsTtl) == WirePacketTypeV2::BEACON)
        {
            srcAddr = Address(LogicalIdToPseudoMac48(logicalSrc));
        }
    }
    MeshMetricTag rxTag;
