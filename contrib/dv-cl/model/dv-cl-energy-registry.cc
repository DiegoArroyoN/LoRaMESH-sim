...
DvClEnergyRegistry::RegisterNode(NodeId id)
{
    ...
    if (it == m_states.end())
    {
        ...
        NS_LOG_INFO("DvClEnergyRegistry: Node " << id << " registered capacity=" << m_capacityMah
                                                << "mAh");
        return;
    }

    it->second.lastUpdate = Simulator::Now();
    NS_LOG_INFO("DvClEnergyRegistry: Node "
                << id << " re-registered keeping remaining=" << it->second.remainingMah << "mAh");
}
