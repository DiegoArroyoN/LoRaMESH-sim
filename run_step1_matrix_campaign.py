#!/usr/bin/env python3
"""
Step 1 full matrix campaign (paper frozen config):
- N in {9,16,25,36,49,64}
- load in {low,medium}
- duty in {off,on(1%)}
- seeds in {1..10}
- total runs: 240

Includes early sanity check after first 12 runs in current execution.
Outputs:
- results_runs.csv
- results_agg.csv
- results.json
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Run Step1 full matrix duty campaign")
...
def extract_row(summary: Dict, run_info: Dict, n_nodes: int, load: str, duty_name: str, seed: int, run_dir: Path) -> Dict:
    ...
    return {
        ...
        "seed": seed,
        "resumed": int(bool(run_info.get("resumed"))),
...
def aggregate(rows: List[Dict]) -> List[Dict]:
    ...
    for (n_nodes, load, duty_profile), rr in sorted(grouped.items()):
        row = {"n_nodes": n_nodes, "N": n_nodes, "load": load, "duty_profile": duty_profile, "duty": duty_profile, "n": len(rr)}
        for m in metrics:
...
def early_sanity(rows: List[Dict], loads: List[str]) -> Dict[str, object]:
    combos = {(r["load"], r["duty"]) for r in rows}
    expected = {(l, d) for l in loads for d in ("duty_off", "duty_on")}
    delivered_nonzero = any(int(r.get("delivered_count", 0)) > 0 for r in rows)
    beacon_ratio_ok = all(0.0 <= float(r.get("beacon_tx_sent_over_scheduled", 0.0)) <= 1.0 + 1e-9 for r in rows)
    return {
        "rows_checked": len(rows),
        "combos_present": sorted(list(combos)),
        "combos_expected": sorted(list(expected)),
        "combos_ok": expected.issubset(combos),
        "delivered_nonzero": delivered_nonzero,
        "beacon_ratio_ok": beacon_ratio_ok,
        "pass": expected.issubset(combos) and delivered_nonzero and beacon_ratio_ok,
    }
...
def main() -> int:
    ...
    outdir = Path(args.outdir).resolve() if args.outdir else BASE_DIR / "validation_results" / f"step1_matrix_240_{ts}"
    ensure_dir(outdir)
    ...
    # Order jobs so first 12 runs cover low/medium x duty_off/on with seeds 1..3 for first N
    jobs = []
    for n in n_values:
        for seed in seeds:
            for load in loads:
    ...
    for idx, (n_nodes, load, duty_name, seed, cfg) in enumerate(jobs, start=1):
        ...
        print(f"[{idx:03d}/{total}] {run_id}")
        res = run_one(run_dir, sim_args, resume=args.resume, git_commit=git_commit)
        row = extract_row(res["summary"], res, n_nodes, load, duty_name, seed, run_dir)
        rows.append(row)
        print(
        ...
        # Periodic write for crash safety
        ...
        if (not early_done) and (args.early_check_runs > 0) and (len(rows) >= args.early_check_runs):
            first_n = n_values[0]
            subset = [r for r in rows if int(r.get("N", -1)) == first_n][: args.early_check_runs]
            if len(subset) >= args.early_check_runs:
                check = early_sanity(subset, loads)
                (outdir / "early_sanity_check.json").write_text(json.dumps(check, indent=2), encoding="utf-8")
                print(f"[EARLY-CHECK] pass={check['pass']} combos_ok={check['combos_ok']} delivered_nonzero={check['delivered_nonzero']} beacon_ratio_ok={check['beacon_ratio_ok']}")
                early_done = True
    ...
    summary = {
        ...
        "matrix": {
            "n_values": n_values,
            "loads": loads,
            "duty_profiles": ["duty_off", "duty_on"],
            "seeds": seeds,
        },
    }
    ...
    print("[OK] step1 matrix completed")
